...
import com.xma.jupiter.entity.User;
...
public class MySQLConnection {
    ...
    public MySQLConnection() throws MySQLException {
        try {
//            Class.forName("com.mysql.cj.jdbc.Driver").newInstance();//reflex, determine the class until running to this line
//            conn = DriverManager.getConnection(MySQLDBUtil.getMySQLAddress());
            Context context = new InitialContext();//load content from the context.xml
    ...
    public void setFavoriteItem(String userId, Item item) throws MySQLException {
        ...
        saveItem(item);
        // method 1
        // String sql = "INSERT IGNORE INTO favorite_records (user_id, item_id) VALUES (" +userId +","+ item.getId()+")";
        // method 2
...
//         String sql = String.format(template, userId, item.getId());
        // possible wrong "INSERT IGNORE INTO favorite_records (user_id, item_id) VALUES ("1111; DROP TABLES;')" // hostile attack
        // SELECT * FROM users WHERE user_id = 1111 OR 1 = 1;
        String sql = "INSERT IGNORE INTO favorite_records (user_id, item_id) VALUES (?, ?)"; // this form help avoid hostile attack
        // IGNORE // ignore the duplicate items
        try {
            PreparedStatement statement = conn.prepareStatement(sql);
            statement.setString(1, userId);//database index start from 1
    ...
    public Map<String, List<Item>> getFavoriteItems(String userId) throws MySQLException {
        ...
        // String sql = "SELECT * FROM favorite_records INNER JOIN items ON favorite_reocrds.item_id = items.id WHERE user_Id = ?";
        try {
            PreparedStatement statement = conn.prepareStatement(sql);
